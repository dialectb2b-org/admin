$(function() {
            ...
            success: function( data ) {
                if(data.length > 0){
                    $.each(data, function( index, value ) {
                        ...
                        cols += '<td><input name="delivered_quantity[]" type="number"  value="'+data[index].quantity+'" class="form-control delievry_quantity" step=".01"/></td>';
                        ...
                        cols += '<input name="unit_price[]" type="hidden"  value="'+data[index].unit_price+'" class="form-control unit_price" step=".01"/>';
                        cols += '<input name="tax_percentage[]" type="hidden"  value="'+data[index].tax_percentage+'" class="form-control tax_per" step=".01"/>';
                        cols += '<input name="tax_value[]" type="hidden"  value="'+data[index].tax_value+'" class="form-control tax_value" step=".01"/>';
                        cols += '<input name="discount_percentage[]" type="hidden"  value="'+data[index].discount_percentage+'" class="form-control discount_per" step=".01"/>(<span class="discount-value-text">'+data[index].total_value+'</span>)';
                        cols += '<input name="total_value[]" type="hidden"  value="'+data[index].total_value+'" class="form-control sub-total" step=".01"/>';
                        ...
                        cols += '<input type="hidden" class="product-code" value="'+data[index].product.code+'"/></td>';
                        newRow.append(cols);
    ...
    function setProduct(data){
        ...
        if(data){
            ...
            if(flag){
                ...
                cols += '<td><input name="delivered_quantity[]" type="number"  value="1" class="form-control delievry_quantity" step=".01"/></td>';
                cols += '<td><input name="unit_id[]" type="hidden" class="form-control" value="'+data.unit_id+'" required>'+data.unit+'</td>';                     
                cols += '<input name="unit_price[]" type="hidden"  value="'+data.rate+'" class="form-control unit_price" step=".01"/>';
                cols += '<input name="tax_percentage[]" type="hidden"  value="'+data.tax_per+'" class="form-control tax_per" step=".01"/>';
                cols += '<input name="tax_value[]" type="hidden"  value="0.00" class="form-control tax_value" step=".01"/>';
                cols += '<input name="discount_percentage[]" type="hidden"  value="0.00" class="form-control discount_per" step=".01"/>(<span class="discount-value-text">0.00</span>)';
                cols += '<input name="total_value[]" type="hidden"  value="0.00" class="form-control sub-total" step=".01"/>';
    ...
    function calculateRowProductData(rowindex){
        ...
        var delievry_quantity = $('table#product-list tbody tr:nth-child(' + (rowindex + 1) + ')').find('.delievry_quantity').val();
        ...
        if(delievry_quantity > quantity){
            Swal.mixin({
                ...
                timer: 3e3,
                timerProgressBar: !0,
                }).fire({ icon: "warning", title: "Delivery qty cannot be greater than ordered quantity" });
            $('table#product-list tbody tr:nth-child(' + (rowindex + 1) + ')').find('.delievry_quantity').val(quantity);    
        }

        if(sales_rate_type == 'Inclusive'){
            var tax_less =(parseFloat(delievry_quantity) * parseFloat(unit_price))/(1 + tax_per/100);
            var tax_value = (parseFloat(delievry_quantity) * parseFloat(unit_price)) - tax_less;
            var original_price = parseFloat(delievry_quantity) * parseFloat(unit_price);
        }
        else if(sales_rate_type == 'Exclusive'){
            var tax_value = unit_price * parseFloat(delievry_quantity)  * parseFloat(tax_per)/100;
            var original_price = parseFloat(delievry_quantity) * parseFloat(unit_price) + parseFloat(tax_value);
            
        }
        
        if(discount_type == 'Flat'){
            var discount_value = parseFloat(discount_per);
            var subtotal = parseFloat(delievry_quantity) * parseFloat(unit_price) + parseFloat(tax_value) - parseFloat(discount_value);
        }
    ...
    $("table#product-list").on('input', '.delievry_quantity', function() {
